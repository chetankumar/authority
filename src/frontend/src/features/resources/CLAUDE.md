...
- **Chat** → same pattern as the editor's start-chat, except the parent is the book (`parentType: "book"`) and there is no selection excerpt. Opens the shared conversation window with no scene attached.
- **Chat rows** → reopen that thread in the same window.
