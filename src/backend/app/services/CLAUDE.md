...
| `ProposalService` | Locate/apply/reject proposals — the **only** code path that mutates on behalf of AI output (plus enrichment exact-match writes); accepted `todo-create` proposals go through `TodoService.create`, with `conversationId` set to the proposing conversation |
