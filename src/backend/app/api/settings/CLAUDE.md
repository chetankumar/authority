...
| POST | `/api/settings/models` | Provider rules: openai-compatible/ollama require baseUrl. apiKey optional for all providers — empty falls back to the provider default env var (anthropic→`ANTHROPIC_API_KEY`, openai→`OPENAI_API_KEY`, gemini→`GOOGLE_API_KEY`, openrouter→`OPENROUTER_API_KEY`). No connectivity test at save |
...
| POST | `/api/settings/models/{id}/test` | Live check: ModelFactory builds the model (resolving `${ENV_VAR}` now) and sends one `"hello model"` completion (~30s timeout). Returns 200 `ModelTestResult {ok, message?, error?, latencyMs?}` — failures are results, not errors. 404 unknown id. Only settings endpoint doing network I/O for a saved model; never mutates app.json |
| GET | `/api/settings/provider-models?provider=&baseUrl=` | Cached model-id suggestions for the Model modal. Reads `{appDataRoot}/provider-models-cache.json` only — never `app.json`, never hits the network |
| POST | `/api/settings/provider-models/sync` | `{ provider, baseUrl?, apiKey? }` → live list from the provider, atomic write to `provider-models-cache.json` only. 422 missing key/baseUrl |
