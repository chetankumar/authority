...
| **Chat** | Tool panel | `POST /conversations {kind:"chat", parent: scene}` → Conversation Modal. If a selection is active, it rides the first message as `context` and renders as a quoted block | The "I'm stuck" button; selection-as-context is the v1 answer to inline markers |
...
| Right pane accordion | Right, 320px | **Notes** (`GET /scenes/{id}/conversations`, kinds note/chat): rows = title · relative time · snippet; click → Conversation Modal. **To-dos** (`GET /scenes/{id}/todos`): checkbox = done (`PATCH`), ✕ = closed; dependency-origin rows carry a ⛓ icon + amber tint; 💬 opens linked conversation. **AI Jobs** (`GET /jobs?scene=`): rows = name · status chip (queued/running spinner/done/failed, live via SSE) · unrecognized-names note; click → its conversation. Amber count badges on headers = open/pending items | The scene's memory: everything ever discussed, owed, or run against this scene, adjacent to the prose but collapsed by default — present, not loud |
...
| **Parts tab:** ordered rows, ↑↓ arrows, ✎, 🗑, [＋ Add part] | ↑↓ → `PATCH {moveBefore/moveAfter}` (response = full ordered list, re-rendered); 🗑 → `DELETE`, 409 → Blocked dialog | Arrows over drag: unambiguous, keyboard-friendly, honest about the linked list underneath |
