...
**Leaving with a running run:** navigation allowed; automatic bookkeeping runs on the server worker regardless; the AI Jobs pane and SSE keep truth. An AI-Job run streams while its Conversation Modal is mounted — **Minimize** keeps it mounted behind a dock chip; **× Close** (confirmed while busy) aborts the client listener and it can be re-sent.
...
**Minimize / dock:** Minimize (or Esc/scrim while generating) keeps the conversation component mounted and the message SSE open; the full modal is replaced by a bottom-right dock chip (title + live status). Click the chip to restore. **× Close** while generating confirms before aborting the client listener; idle close/click-out just closes.
...
| **Minimize** | Header – · Esc/scrim while generating → dock chip (bottom-right); component stays mounted, message SSE keeps running; click chip to restore | Long AI-Jobs must not block writing; Close would abort the client listener |
| Close / click-out | Idle: just closes. While generating: × confirms before abort; Esc/scrim minimize instead. Every message already persisted on send; pending proposals survive and badge the accordion | No "save conversation?" anxiety; hard Close is explicit so minimize is the safe escape |
