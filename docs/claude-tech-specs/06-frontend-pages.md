...
| **＋ Add scene** floating button (primary) | Scene Modal (create); Sequence defaults to last trunk scene → The End (Start → The End if the book is empty). Author can clear either side to leave unplaced. `POST /scenes` → node appears via cache patch, brief 200ms fade-in | Creation belongs on the map where placement is being imagined; appending is the common act |
...
Toolbar: left — segmented filter **All / Placed / Floating** + **Archived** toggle; right — [Columns ▾] popover (checkbox list of available columns) + [＋ Add scene] (primary). Add scene opens Scene Modal (create) with Sequence defaulted to last trunk scene → The End (Start → The End if empty); either side is clearable to leave the new scene unplaced.
...
| Prev/Next selects | Splice semantics server-side; `affectedScenes` in the response patch the graph. **Create defaults:** graph/table Add scene → Previous = last trunk scene (or Start if empty), Next = The End; editor "New next scene" → Previous = this scene, Next = its current successor (The End when this is the trunk tail). Either side remains clearable ("— unplaced —"). | Placement *is* authorship of structure; dropdowns beat drag precisely. Defaulting to append keeps drafting momentum without blocking a floating create |
...
| **← Previous / Next →** | Bottom bar, right | Save current, then: neighbor exists → navigate + **scroll to top**; Next with no neighbor → Scene Modal (create) with Previous prefilled to this scene and Next snapped to its successor (The End when this is the last trunk scene) → on save, navigate into the new scene. Prev with no neighbor → disabled with tooltip "This is the first scene" | Drafting momentum: write, next, write. The prefilled-create turns "what comes next?" into one gesture; snapping Next keeps The End on the chain; scroll-to-top starts each scene at its beginning |
...
**Leaving with a running run:** navigation inside the book is allowed and does **not** abort in-flight Chat / AI-Job streams. Message SSE is owned by a book-scoped session store in the app shell (same lifetime as the book EventSource), not by the modal. Minimize (or navigating to another scene/page) unmounts the modal shell only; tokens keep landing in the store and a dock chip stays put. **× Close** (confirmed while busy) aborts that conversation's listener; leaving the book or a full reload aborts leftover streams. Automatic bookkeeping runs on the server worker regardless. The AI Jobs pane still reflects status via the book `conversation` event.
...
**Header:** editable title (controlled; blur → `PATCH /conversations/{id}`; full title stored — list UIs truncate with hover; auto-filled from utility-model **3–5 word** semantic title when still Untitled after first send — SSE `title`) · relative timestamp · right cluster: **model select** + **"AI participant" switch** · **Delete** · **Minimize (–)** · ×.
...
**Jump to latest:** the message list does **not** auto-follow stream/token updates. A floating ↓ (bottom-right of the scroll pane) appears when content sits below the fold; click scrolls the list container to the latest. Send scrolls once so the new turn is in view, then the viewport stays put while tokens append.
...
**Minimize / dock:** Minimize (or Esc/scrim while generating) unmounts the modal shell only. The `POST /messages` SSE is owned by a book-scoped session store in the app shell, so tokens keep arriving. Other open conversations become stacked dock chips (bottom-right, amber ring while busy). Click a chip to restore that thread (at most one expanded modal). Opening Chat / an AI-Job / a Notes row **adds** a session and focuses it — it does not abort a minimized stream. Scene and page navigation inside the book keep streams alive. **× Close** while generating confirms before aborting that one listener; idle close/click-out just closes that session. Leaving `/book/{id}` disposes the store and aborts leftover streams.
...
| Send | `POST /messages` — switch off: plain append (the note path). Switch on: SSE stream renders tokens live; final `message` event replaces the streaming buffer with the persisted message + proposal cards; `error` event → inline danger row + composer re-enabled. First send while Untitled → utility model names the thread (dedicated prompt; full reply stored); header updates via `title` event. Scrolls the message list to the bottom once; does not keep following | |
| **↓ Scroll to latest** | Floating button over the message list when not at the bottom; `scrollTo` on the list container (never the outer modal). Hidden when already near the bottom (~40px) | Reading a growing stream must not yank the viewport; the author opts in to jump down |
...
| Accept / Reject / Accept all | `POST /proposals/{id}/accept|reject`; Accept-all loops sequentially, halting to surface any not-found. Each resolution appends a system note to the thread; when AI is on and no cards remain pending after Accept, the modal auto-sends continue | The approval gate made tactile: every AI-initiated change is a card the author physically dismisses or applies; not-found is shown, never silently skipped |
| **Minimize** | Header – · Esc/scrim while generating → dock chip (bottom-right, stacked when several are open); modal unmounts, message SSE stays on the book-scoped store; click chip to restore | Long AI-Jobs must not block writing or a second chat; Close would abort that listener |
| Close / click-out | Idle: just closes that session. While generating: × confirms before abort; Esc/scrim minimize instead. Every message already persisted on send; pending proposals survive and badge the accordion | No "save conversation?" anxiety; hard Close is explicit so minimize is the safe escape |
