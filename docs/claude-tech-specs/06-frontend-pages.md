...
**Leaving with a running chat:** you can change scene or page without stopping the reply. The chat window is just a view; App holds the live connection for as long as this book is open. Minimize (or walk away) and a chip in the corner keeps showing progress. **× Close** (confirmed while generating) stops **that** chat. Leaving the book, or refreshing, stops all of them. Automatic bookkeeping still runs on the server worker. The AI Jobs list still updates from the book event channel.
...
**Minimize / dock:** Minimize (or Esc/scrim while generating) hides the window. The live reply keeps running because App owns the connection, not the window. Other open chats become stacked chips in the bottom-right (amber ring while generating). Click a chip to bring that chat back — only one window is expanded at a time. Starting Chat or an AI-Job adds another chat; it does not stop the one in the corner. Changing scene or page inside the book keeps them all alive. **× Close** while generating confirms, then stops that one reply. Idle close just dismisses that chat. Leaving the book, or refreshing, stops every live reply.
...
| **Minimize** | Header – · Esc/scrim while generating → chip in the bottom-right (stacked if several chats are open); the window hides, the reply keeps running; click the chip to restore | A long AI-Job must not block writing or a second chat; Close would stop the reply |
| Close / click-out | Idle: dismiss that chat. While generating: × confirms before stopping it; Esc/scrim minimize instead. The user message is already saved; pending proposals survive and badge the accordion | No "save conversation?" anxiety; hard Close is explicit so minimize is the safe escape |
