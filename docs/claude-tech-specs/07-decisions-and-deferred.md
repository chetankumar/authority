...
13. Enrichment: leave-scene (`POST …/enrich/auto`) + on-demand redo. Summary and character-matching are independent model calls against independent settings slots (`sceneSummaryModelId` / `characterParsingModelId`, each falling back to `utilityModelId`) — not one shared "utility model" call. Clear matches update summary/`characters` (with involvement) under bookkeeping toggles. Unclear cases (unmatched names, ambiguity, minor-character judgment) escalate to a chat; character sheet creates only via author-accepted `character-create` proposals — never silent auto-create.
...
29. **Scene Table optional columns use AG Grid `initialHide`, not `hide`.** In ColDef, `hide` is *stateful* — AG Grid re-applies it whenever column definitions are reconciled, which undoes runtime `applyColumnState` from the Columns ▾ chooser on the next React re-render (optional columns flickered off; only one could appear selected). `initialHide` applies only when the column is first created; after that, visibility is controlled exclusively by `applyColumnState` (author toggles) and `tableColumnState` restore from `ui.json`. Default-visible columns omit both flags. Implementation: [`ScenesTablePage.tsx`](../../src/frontend/src/features/table/ScenesTablePage.tsx).
30. **Search index is derived, gitignored Chroma** (`{book}/search-index/`). Exception to JSON-only manuscript persistence, same class as generated mp3s. Not committed; rebuild restores it. Explicit index only (not autosave). Header search is Q&A (answer + hits), not a saved conversation; the answer model sees matched slices, not whole scenes.
