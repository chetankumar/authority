...
Input for each call: scene prose. Character-parsing also gets the cast directory (names/aliases) **and the scene's current `characters` rows** (ids + involvement), so a redo can preserve/refine author-edited involvement instead of rebuilding from prose alone.

- **Clear cases:** Summary call overwrites `scene.summary`. Character call sets `characters` from prose + existing scene rows (keep/refine involvement that still fits; add newly clear matches; drop only when prose no longer supports them). Field changes emit `scene-updated` SSE.
