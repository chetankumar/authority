...
- **Clear cases:** the model calls `set_scene_summary` or `set_scene_characters` — execute tools that write through `SceneService.update_scene` (validating character ids, preserving `contentHash`/`wordCount`, emitting `scene-updated`). The persisted assistant message includes a recap of what was written (the summary text, or character names + involvement), so the thread is not blank after a tool-only reply. The run ends `done`.
